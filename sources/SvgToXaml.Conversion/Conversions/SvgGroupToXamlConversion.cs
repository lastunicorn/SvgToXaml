...
internal class SvgGroupToXamlConversion : SvgContainerToXamlConversion<SvgGroup, Canvas>
{
    ...
    protected override void OnExecuted()
    {
        ...
        if (isZeroSize)
            ConversionContext.Issues.AddWarning($"[{SvgElement.ElementName}] Zero-size canvas detected.");

        bool isEmpty = XamlElement.Children.Count == 0;
        if (isEmpty)
            ConversionContext.Issues.AddWarning($"[{SvgElement.ElementName}] Empty canvas detected.");
    }
}